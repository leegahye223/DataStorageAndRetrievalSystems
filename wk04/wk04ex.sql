...
In this exercise you will create views, indexes, and a new user, and you will 
grant a limited set of privileges to the new users.

Part 1 - Set Up

1. Open your terminal and change to the directory that has the exercise files.
...
1. Create a table named `person`. It should have 3 attributes:
    - An `id` primary key (in PostgreSQL, use the `SERIAL` data type)
...
2. Create a table named `secret`. It should have 4 attributes:
    ...
    - A required `level` small integer
        - This will signify how sensitive the secret is. A zero means, it's 
          not really a secret. A 1 means it should not be mentioned. Anything
          higher means it is top secret!
...
/* 
3. Use the "Import" link to execute the INSERT statements in the 
`names.sql` file.
*/

/*
4. Use the `EXPLAIN` keyword for all of the following queries. Enter them all
in the same execution text box.
...
EXPLAIN SELECT * FROM person2 LIMIT 100;

EXPLAIN SELECT * FROM person2 ORDER BY name LIMIT 100;

EXPLAIN SELECT * FROM person2 WHERE dob > '1900-01-01' ORDER BY name LIMIT 100;

EXPLAIN SELECT * FROM person2 WHERE name = 'Zclvbwicgeevwpiyodbaamu';

/*
5. Look at the results. Pay attention to the 1st row of each qyery plan. 
The second number (after the ...) is the total expected cost.
