...
CREATE TABLE has_genre(
    ...
    genreid INTEGER REFERENCES genre ON DELETE CASCADE,
...
CREATE TABLE film(
    ...
    rating REAL,
    imdb_votes REAL,
    directorid INTEGER REFERENCES director
);
...
INSERT INTO film
SELECT id, title, year, duration, budget, boxoffice, genre, rating, imdb_votes,directorid
FROM old_film;
...
SELECT label, group_concat(title)
FROM film, genre, has_genre
WHERE film.id = has_genre.filmid
...
SELECT name, count(DISTINCT label)
FROM film, director, genre, has_genre 
WHERE film.directorid = director.id
AND film.id = has_genre.filmid
AND genre.id = has_genre.genreid
